<?php
    ...
    $image = $db->query("SELECT * FROM image WHERE id = {$_GET['id']}" )->fetch(PDO::FETCH_ASSOC);
    
    if (isset($_GET['action']) && $_GET['action']='delete') {
        unlink('uploads/'.$image['image']);
        $db->exec("DELETE FROM image WHERE id = {$_GET['id']}");
        header('Location: /');
    }
?>
<!DOCTYPE html>
            ...
            <div class="col-xs-12 col-sm-6 col-sm-offset-3">
                <figure class="works-item">
                    ...
                    <figcaption class="works-item_text">
                        ...
                        <h3>
                            <a href="/#images"><?=htmlentities($image['title'])?></a>
                        </h3>
                    </figcaption>
                </figure>
                ...
                <div>
                    <a class="btn btn-info" href="/template_add_edit_image.php?id=<?=$image['id']?>">Edit image</a>
                    <a class="btn btn-danger" id="del" href="/template_view_image.php?id=<?=$image['id']?>&action=delete">Delete image</a>
                </div>
            </div>
            <!-- EO Image entry -->
        </div>
